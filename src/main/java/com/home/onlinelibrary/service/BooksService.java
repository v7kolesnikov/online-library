...
public class BooksService {
    ...
    public BooksService(BooksRepository repository, ResourcesRepository resourceRepository, UploadFileService uploadFileService) {
    ...
    public void saveToCache(String modifiedName, byte[] data) {
        File store = uploadFileService.getFilePath(modifiedName, "files");
    ...
    private void deleteFile(String modifiedIconName) {
        if (modifiedIconName != null && !FilenameUtils.getExtension(modifiedIconName).isEmpty()) {
            File storedFile = new File(context.getRealPath("/files/" + File.separator + modifiedIconName));
